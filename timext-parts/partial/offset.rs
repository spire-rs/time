...
/// An `InOffsetDateTime` struct represents an incomplete [OffsetDateTime] struct.
...
impl PartOffsetDateTime {
    /// Replaces the offset.
    pub fn replace_offset(self, offset: Option<UtcOffset>) -> Result<Self, PartRange> {
        Ok(Self::new(self.datetime, offset))
    }

    /// Replaces the date and time.
    pub fn replace_date_time(self, datetime: PartPrimitiveDateTime) -> Result<Self, PartRange> {
...
impl Partial for PartOffsetDateTime {
    ...
    fn into_complete(self) -> Result<Self::Complete, PartRange> {
        let error: PartRange = NoComponent::new("offset").into();
